...
def process_one_file(file):
    ...
    ia_id = os.path.basename(file).split(sep="_", maxsplit=1)[0]

    print("found an id_id of", ia_id)
    ...
    people_date_sentence = {}

    def insert_person_date_sentence(person, date, sentence):
        if people_date_sentence.get(date) is None:
            people_date_sentence[date] = {}
        if people_date_sentence[date].get(person) is None:
            people_date_sentence[date][person] = []
        people_date_sentence[date][person].append({'ia_id': ia_id, 'leaf': pagecount, 'sentence': sentence })
            ...
            for sentence in par:
                ...
                if len(found_dates) == 1:
                    ...
                    for t in found_things:
                        ...
                        insert_person_date_sentence(t, d, sentence)
    ...
    with open(outfile, 'w', newline='') as csvfile:
        ...
        w.writerow(['person', 'score', 'breadth', 'depth'])

        sorted_list = ((k, scores[k]) for k in sorted(scores, key=scores.get, reverse=True))

        for k, v in sorted_list:
            w.writerow([k, v, details[k][0], details[k][1]]) # XXX fixme, surely there's a better way to flatten details
    ...
    with open(outfile, 'w', newline='') as csvfile:
        ...
        for d in sorted(people_date):
            ...
            for t, tv in sorted_things:
                w.writerow([d, t, people_date[d][t]])

    outfile = file.replace('.xml', '.thing-date-sentence.csv')
    outfile = outfile.replace('.0.', '.')
    if outfile == file:
        raise NameError('could not form the name of the outfile')
    ...
    with open(outfile, 'w', newline='') as csvfile:
        w = csv.writer(csvfile, delimiter='\t')
        w.writerow(['date', 'person', 'ia_id', 'leaf', 'sentence'])

        for d in sorted(people_date):
            sorted_things = ((t, people_date[d][t]) for t in sorted(people_date[d], key=people_date[d].get, reverse=True))
            for t, tv in sorted_things:
                for row in people_date_sentence[d][t]:
                    w.writerow([d, t, row['ia_id'], row['leaf'], row['sentence']])
